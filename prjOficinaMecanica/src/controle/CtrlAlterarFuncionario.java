...
import controle.CtrlAlterarPeca.Status;
...
public class CtrlAlterarFuncionario {
	//
	// ATRIBUTOS
	//
	public enum Status {
...
//	private UIPeca uiPeca;
	 ...
	 */
	public CtrlAlterarFuncionario(CtrlManterFuncionarios ctrl, Funcionario p) throws ControleException, DadosException {
...
//		ctrlManterDepartamentos.terminarCasoDeUsoAlterarDepartamento();
	 ...
	 * 
	 */
	public void alterar(int matricula, String funcao) throws DadosException, ControleException {
		// Se o controlador não tinha ativado uma inclusao, não faço nada!
		if(this.status != Status.ALTERANDO)
			throw new ControleException(new ErroDeControle("Não é possível concluir uma operação de alteração"));
		// Atualizo os campos
		this.atual.setMatricula(matricula);
		this.atual.setFuncao(funcao);
